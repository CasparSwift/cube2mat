...
static std::vector<int> run_lengths(const std::vector<int>& s) {
    std::vector<int> lens;
    int prev = 0;
    int cur = 0;
    for (int v : s) {
...
void cube2mat_run_length_entropy_signret(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            ssize_t uniq = counts.size();
            res_ptr[i * d1 + j] = (uniq > 1)
                ? static_cast<float>(H / std::log2(static_cast<double>(uniq)))
                : 0.0f;
        }
    }
}
...
void bind_cube2mat_run_length_entropy_signret(py::module& m) {
    m.def("cube2mat_run_length_entropy_signret", &cube2mat_run_length_entropy_signret,
          py::arg("result"), py::arg("cubes_map"),
          "Shannon entropy of sign-run length distribution for log returns in RTH, normalized by log2(#unique lengths).");
}
