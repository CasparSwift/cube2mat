...
void cube2mat_xcorr_sign_volume_leadlag_lag(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            float best_corr = std::numeric_limits<float>::quiet_NaN();
            float best_lag  = std::numeric_limits<float>::quiet_NaN();
            for (int lag = -K; lag <= K; ++lag) {
                ...
                // drop first element due to initial NaN in sgn
                start_x += 1;
                start_y += 1;
                len -= 1;
                ...
                if (!std::isnan(c) && (std::isnan(best_corr) || std::fabs(c) > std::fabs(best_corr))) {
                    best_corr = static_cast<float>(c);
                    best_lag  = static_cast<float>(lag);
                }
            }
            res_ptr[i*d1 + j] = best_lag;
        }
    }
}

void bind_cube2mat_xcorr_sign_volume_leadlag_lag(py::module& m) {
    m.def("cube2mat_xcorr_sign_volume_leadlag_lag", &cube2mat_xcorr_sign_volume_leadlag_lag,
          py::arg("result"), py::arg("cubes_map"),
          "Lag of peak |corr(sign(ret), volume shift)| over [-5,5] in RTH.");
}
