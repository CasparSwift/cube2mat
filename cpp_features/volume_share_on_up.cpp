...
void cube2mat_volume_share_on_up(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
...
#pragma omp parallel for collapse(2)
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            float upv=0.0f, totv=0.0f; float prev=std::numeric_limits<float>::quiet_NaN(); bool have=false;
            ...
            res_ptr[i*d1+j]=(totv>0.0f && have)?(upv/totv):std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_volume_share_on_up(py::module& m){
    m.def("cube2mat_volume_share_on_up", &cube2mat_volume_share_on_up,
          py::arg("result"), py::arg("cubes_map"),
          "Share of total interval_volume on up bars");
}
