...
void cube2mat_vwap_twap_diff(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            if(twap_cnt>0 && vwap_den>0.0){
                ...
                res_ptr[i*d1+j]=static_cast<float>(vw - twap);
            }else res_ptr[i*d1+j]=std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_vwap_twap_diff(py::module& m){
    m.def("cube2mat_vwap_twap_diff", &cube2mat_vwap_twap_diff,
          py::arg("result"), py::arg("cubes_map"),
          "VWAP minus TWAP within 09:30–15:59; NaN if insufficient.");
}
