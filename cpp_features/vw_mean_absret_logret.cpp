...
void cube2mat_vw_mean_absret_logret(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
...
#pragma omp parallel for collapse(2)
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            res_ptr[i*d1+j]=(den>0.0f)?(num/den):std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_vw_mean_absret_logret(py::module& m){
    m.def("cube2mat_vw_mean_absret_logret", &cube2mat_vw_mean_absret_logret,
          py::arg("result"), py::arg("cubes_map"),
          "Volume-weighted mean absolute log return");
}
