...
void cube2mat_log_absret_on_logvolume_beta(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 1; t < d2; ++t) {
                ...
                if (absr <= 0.f || std::isnan(absr)) continue;
                double x = std::log(v);
                double y = std::log(absr);
...
void bind_cube2mat_log_absret_on_logvolume_beta(py::module& m) {
    m.def("cube2mat_log_absret_on_logvolume_beta", &cube2mat_log_absret_on_logvolume_beta,
          py::arg("result"), py::arg("cubes_map"),
          "Impact exponent: slope of log|logret| ~ log(volume).");
}
