...
void cube2mat_n_fano_factor(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            if (cnt >= 2) {
                ...
                if (mean > 0.0 && std::isfinite(var)) {
                    res_ptr[i * d1 + j] = static_cast<float>(var / mean);
                    continue;
                }
            }
...
void bind_cube2mat_n_fano_factor(py::module& m) {
    m.def("cube2mat_n_fano_factor", &cube2mat_n_fano_factor,
          py::arg("result"), py::arg("cubes_map"),
          "Fano factor of n across intraday bars: Var(n)/Mean(n).");
}
