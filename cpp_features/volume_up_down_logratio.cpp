...
void cube2mat_volume_up_down_logratio(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
...
#pragma omp parallel for collapse(2)
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            float upv=0.0f, dnv=0.0f; float prev=std::numeric_limits<float>::quiet_NaN(); bool have=false;
            for(ssize_t t=0;t<d2;++t){
                ...
                if(have){
                    ...
                    if(ret>0) upv+=v; else if(ret<0) dnv+=v;
                }
                have=true; prev=c;
            }
            res_ptr[i*d1+j]=std::log((upv+EPS)/(dnv+EPS));
        }
    }
}

void bind_cube2mat_volume_up_down_logratio(py::module& m){
    m.def("cube2mat_volume_up_down_logratio", &cube2mat_volume_up_down_logratio,
          py::arg("result"), py::arg("cubes_map"),
          "Log-ratio of volumes on up vs down bars");
}
