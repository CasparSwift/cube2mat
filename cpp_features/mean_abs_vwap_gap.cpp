...
void cube2mat_mean_abs_vwap_gap(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 0; t < d2; ++t) {
                ...
                sum += std::fabs(static_cast<double>(c) - static_cast<double>(w));
                ++cnt;
            }
...
void bind_cube2mat_mean_abs_vwap_gap(py::module& m) {
    m.def("cube2mat_mean_abs_vwap_gap", &cube2mat_mean_abs_vwap_gap,
          py::arg("result"), py::arg("cubes_map"),
          "Mean of |close - vwap| during RTH; NaN if no valid bars.");
}
