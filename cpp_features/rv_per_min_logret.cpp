...
void cube2mat_rv_per_min_logret(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            res_ptr[i*d1+j]=(cnt>0)?static_cast<float>(sum/TOTAL_MIN):std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_rv_per_min_logret(py::module& m){
    m.def("cube2mat_rv_per_min_logret", &cube2mat_rv_per_min_logret,
          py::arg("result"), py::arg("cubes_map"),
          "RV per-minute intensity: RV divided by session total minutes (389).");
}
