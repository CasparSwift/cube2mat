...
void cube2mat_pre_std_ret(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            if (d2 < RTH_START+1){ res_ptr[i*d1+j]=std::numeric_limits<float>::quiet_NaN(); continue; }
            double sum=0.0,sum2=0.0; int cnt=0;
            for (ssize_t t=1; t<RTH_START; ++t){
...
void bind_cube2mat_pre_std_ret(py::module& m){
    m.def("cube2mat_pre_std_ret", &cube2mat_pre_std_ret,
          py::arg("result"), py::arg("cubes_map"),
          "Pre-market std of intraday returns (pct_change of close) 00:00–09:29; NaN if <3 returns.");
}
