...
void cube2mat_marubozu_density(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    if(pbuf.ndim!=3||hbuf.ndim!=3||lbuf.ndim!=3) throw std::runtime_error("inputs must be 3D");
    ...
    const float alpha=0.10f; // tiny shadow
    const float beta=0.80f;  // long body
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            for(ssize_t t=1;t<d2;++t){
                ...
                if(std::isnan(open)||std::isnan(close)||std::isnan(high)||std::isnan(low)||!(high>low))
                    continue;
                float range = high - low;
                if(!(range>0.f)) continue;
                float body = std::fabs(close - open);
                float upper = high - std::fmax(open, close);
                float lower = std::fmin(open, close) - low;
                if(upper<0.f) upper=0.f;
                if(lower<0.f) lower=0.f;
                ++tot;
                if(body/range >= beta && upper/range <= alpha && lower/range <= alpha) ++cnt;
            }
...
void bind_cube2mat_marubozu_density(py::module& m){
    m.def("cube2mat_marubozu_density", &cube2mat_marubozu_density,
          py::arg("result"), py::arg("cubes_map"),
          "Density of long-body tiny-shadow bars (alpha=0.10, beta=0.80) in RTH.");
}
