...
void cube2mat_near_low_time_share_10bp(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
                                       const py::dict& cubes_map) {
    if (!cubes_map.contains("last_price") || !cubes_map.contains("interval_low")) {
        throw std::runtime_error("cubes_map must contain 'last_price' and 'interval_low'");
    }
    ...
    if (pbuf.ndim != 3 || lbuf.ndim != 3) {
        throw std::runtime_error("inputs must be 3D arrays");
    }
    if (pbuf.shape[0] != lbuf.shape[0] || pbuf.shape[1] != lbuf.shape[1] || pbuf.shape[2] != lbuf.shape[2]) {
...
void bind_cube2mat_near_low_time_share_10bp(py::module& m) {
    m.def("cube2mat_near_low_time_share_10bp", &cube2mat_near_low_time_share_10bp,
          py::arg("result"), py::arg("cubes_map"),
          "Fraction of bars with close near session LOW within 10bp.");
}
