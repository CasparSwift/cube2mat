...
void cube2mat_impact_slope_price_cumvol(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            double cum=0.0; double sx=0.0, sy=0.0, sxx=0.0, sxy=0.0; int cnt=0;
            for(ssize_t t=0;t<d2;++t){
                float p=price_ptr[base+t];
                float v=vol_ptr[base+t];
                if(std::isnan(p)||std::isnan(v)) continue;
                cum += v;
                double x=cum; double y=p - anchor;
...
void bind_cube2mat_impact_slope_price_cumvol(py::module& m){
    m.def("cube2mat_impact_slope_price_cumvol", &cube2mat_impact_slope_price_cumvol,
          py::arg("result"), py::arg("cubes_map"),
          "OLS slope of (close - first_open) on cumulative volume within 09:30–15:59; NaN if insufficient.");
}
