...
void cube2mat_close_on_logvolume_beta(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 0; t < d2; ++t) {
                ...
                x.push_back(std::log(static_cast<double>(v)));
...
void bind_cube2mat_close_on_logvolume_beta(py::module& m) {
    m.def("cube2mat_close_on_logvolume_beta", &cube2mat_close_on_logvolume_beta,
          py::arg("result"), py::arg("cubes_map"),
          "OLS slope of close on log(volume) within 09:30–15:59; NaN if insufficient.");
}
