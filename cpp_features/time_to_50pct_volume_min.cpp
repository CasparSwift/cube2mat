...
void cube2mat_time_to_50pct_volume_min(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            if (k >= 0) {
                res_ptr[i * d1 + j] = static_cast<float>((k + 1) * 5.0);
            } else {
                res_ptr[i * d1 + j] = std::numeric_limits<float>::quiet_NaN();
            }
        }
...
void bind_cube2mat_time_to_50pct_volume_min(py::module& m) {
    m.def("cube2mat_time_to_50pct_volume_min", &cube2mat_time_to_50pct_volume_min,
          py::arg("result"), py::arg("cubes_map"),
          "Absolute minutes required to reach 50% of session volume.");
}
