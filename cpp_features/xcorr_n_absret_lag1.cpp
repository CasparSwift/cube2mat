...
void cube2mat_xcorr_n_absret_lag1(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 0; t < d2 - 1; ++t) {
                ...
                if (std::isnan(n) || std::isnan(p0) || std::isnan(p1) || !(p0 > 0.f) || !(p1 > 0.f))
                    continue;
...
void bind_cube2mat_xcorr_n_absret_lag1(py::module& m) {
    m.def("cube2mat_xcorr_n_absret_lag1", &cube2mat_xcorr_n_absret_lag1,
          py::arg("result"), py::arg("cubes_map"),
          "Lead-lag corr: trades count leads |logret| by 1 bar.");
}
