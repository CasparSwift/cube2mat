...
void cube2mat_rvol_over_sqrt_trades(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            double rv = 0.0; double trades = 0.0;
            bool any = false;
            for (ssize_t t = 1; t < d2; ++t) {
                float p0 = price_ptr[base + t - 1];
                float p1 = price_ptr[base + t];
                float v = vol_ptr[base + t];
                if (p0 > 0.f && p1 > 0.f && v > 0.f && !std::isnan(p0) && !std::isnan(p1) && !std::isnan(v)) {
            ...
            if (any && trades > 0.0) {
                res_ptr[i*d1 + j] = static_cast<float>(std::sqrt(rv) / std::sqrt(trades));
            } else {
...
void bind_cube2mat_rvol_over_sqrt_trades(py::module& m) {
    m.def("cube2mat_rvol_over_sqrt_trades", &cube2mat_rvol_over_sqrt_trades,
          py::arg("result"), py::arg("cubes_map"),
          "sqrt(RV)/sqrt(total trades) invariance proxy using interval_volume.");
}
