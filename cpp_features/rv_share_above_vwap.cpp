...
void cube2mat_rv_share_above_vwap(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            double total=0.0; double above=0.0; int cnt=0;
            for(ssize_t t=1;t<d2;++t){
                float p0=price_ptr[base+t-1];
                float p1=price_ptr[base+t];
                float w1=vwap_ptr[base+t];
                if(std::isnan(p0)||std::isnan(p1)||std::isnan(w1)||!(p0>0.f)||!(p1>0.f)) continue;
                double r=std::log(static_cast<double>(p1)/p0); if(!std::isfinite(r)) continue;
                double r2=r*r; total+=r2; ++cnt;
                if(p1> w1) above+=r2;
            }
            res_ptr[i*d1+j]=(cnt>0 && total>0.0)?static_cast<float>(above/total):std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_rv_share_above_vwap(py::module& m){
    m.def("cube2mat_rv_share_above_vwap", &cube2mat_rv_share_above_vwap,
          py::arg("result"), py::arg("cubes_map"),
          "Share of realized variance (log ret) contributed while close>vwap.");
}
