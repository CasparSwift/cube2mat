...
void cube2mat_iqr_logret(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            res_ptr[i * d1 + j] = q3 - q1;
        }
    }
}

void bind_cube2mat_iqr_logret(py::module& m) {
    m.def("cube2mat_iqr_logret", &cube2mat_iqr_logret,
          py::arg("result"), py::arg("cubes_map"),
          "Interquartile range of intraday log returns.");
}
