...
void cube2mat_diff_close_vwap_on_volume_beta(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
                                             const py::dict& cubes_map) {
    if (!cubes_map.contains("last_price") || !cubes_map.contains("interval_vwap") ||
        !cubes_map.contains("interval_volume")) {
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            std::vector<double> x,y; x.reserve(d2); y.reserve(d2);
            for(ssize_t t=0;t<d2;++t){
                float c=price_ptr[base+t];
                float w=vwap_ptr[base+t];
                float v=vol_ptr[base+t];
                if(std::isnan(c)||std::isnan(w)||std::isnan(v)) continue;
                x.push_back(static_cast<double>(v));
                y.push_back(static_cast<double>(c-w));
            }
            size_t m=x.size();
            if(m<2){ res_ptr[i*d1+j]=std::numeric_limits<float>::quiet_NaN(); continue; }
            double xm=0.0, ym=0.0; for(size_t t=0;t<m;++t){xm+=x[t]; ym+=y[t];}
            xm/=m; ym/=m; double sxx=0.0,sxy=0.0; for(size_t t=0;t<m;++t){double xd=x[t]-xm; double yd=y[t]-ym; sxx+=xd*xd; sxy+=xd*yd;}
...
void bind_cube2mat_diff_close_vwap_on_volume_beta(py::module& m){
    m.def("cube2mat_diff_close_vwap_on_volume_beta", &cube2mat_diff_close_vwap_on_volume_beta,
          py::arg("result"), py::arg("cubes_map"),
          "OLS slope of (close - vwap) on volume within 09:30–15:59.");
}
