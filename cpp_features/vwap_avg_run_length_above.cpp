...
void cube2mat_vwap_avg_run_length_above(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
...
#pragma omp parallel for collapse(2)
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            ssize_t run=0; double sumlen=0.0; ssize_t count=0;
            for(ssize_t t=0;t<d2;++t){
                float c=price_ptr[base+t];
                float w=vwap_ptr[base+t];
                if(std::isnan(c) || std::isnan(w)){
                    if(run>0){ sumlen+=run; ++count; run=0; }
                    continue;
                }
                if(c>w){ ++run; }
                else{
            ...
            if(run>0){ sumlen+=run; ++count; }
            res_ptr[i*d1+j] = (count>0)? static_cast<float>(sumlen/count)
                                       : std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_vwap_avg_run_length_above(py::module& m){
    m.def("cube2mat_vwap_avg_run_length_above", &cube2mat_vwap_avg_run_length_above,
          py::arg("result"), py::arg("cubes_map"),
          "Average length of contiguous runs with close>vwap in RTH.");
}
