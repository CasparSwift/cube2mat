...
void cube2mat_spearman_close_n(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    auto pbuf=price_arr.request(); auto nbuf=n_arr.request();
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            for(ssize_t t=0;t<d2;++t){
                float c=price_ptr[base+t]; float nval=n_ptr[base+t];
                if(std::isnan(c)||std::isnan(nval)) continue;
                xvec.push_back(c); yvec.push_back(nval);
            }
            size_t n=xvec.size();
...
void bind_cube2mat_spearman_close_n(py::module& m){
    m.def("cube2mat_spearman_close_n", &cube2mat_spearman_close_n,
          py::arg("result"), py::arg("cubes_map"),
          "Spearman rank correlation between close and n within 09:30–15:59.");
}
