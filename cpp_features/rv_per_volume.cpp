...
void cube2mat_rv_per_volume(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
...
#pragma omp parallel for collapse(2)
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            float sumsq=0.0f; float totv=0.0f; float prev=std::numeric_limits<float>::quiet_NaN(); bool have=false;
            ...
            res_ptr[i*d1+j]=(totv>0.0f && have)?(sumsq/totv):std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_rv_per_volume(py::module& m){
    m.def("cube2mat_rv_per_volume", &cube2mat_rv_per_volume,
          py::arg("result"), py::arg("cubes_map"),
          "Realized variance divided by total interval_volume");
}
