...
void cube2mat_absret_sqrtvol_slope(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
        ...
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 1; t < d2; ++t) {
                ...
                if (std::isnan(p0) || std::isnan(p1) || std::isnan(v) || !(p0 > 0.f) || !(p1 > 0.f) || !(v > 0.f))
                    continue;
                float ret = (p1 - p0) / p0;
                if (std::isnan(ret)) continue;
                double x = std::sqrt(v);
                double y = std::fabs(ret);
...
void bind_cube2mat_absret_sqrtvol_slope(py::module& m) {
    m.def("cube2mat_absret_sqrtvol_slope", &cube2mat_absret_sqrtvol_slope,
          py::arg("result"), py::arg("cubes_map"),
          "OLS slope of |ret| on sqrt(volume) within 09:30–15:59; NaN if insufficient.");
}
