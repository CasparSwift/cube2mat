...
static bool solve_quad_beta2(const std::vector<float>& y, double& b2){
    size_t n=y.size();
    if(n<3) return false;
    ...
    for(size_t k=0;k<n;++k){
        ...
        double t2=t*t;
        sum_t+=t; sum_t2+=t2; sum_t3+=t2*t; sum_t4+=t2*t2;
        double yv=y[k];
        sum_y+=yv; sum_ty+=t*yv; sum_t2y+=t2*yv;
    }
    ...
    b2=det2/det; return true;
}

void cube2mat_trend_quad_beta2(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
...
#pragma omp parallel for collapse(2)
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            double b2; if(!solve_quad_beta2(y,b2)) res_ptr[i*d1+j]=std::numeric_limits<float>::quiet_NaN();
            else res_ptr[i*d1+j]=static_cast<float>(b2);
        }
    }
}

void bind_cube2mat_trend_quad_beta2(py::module& m){
    m.def("cube2mat_trend_quad_beta2", &cube2mat_trend_quad_beta2,
          py::arg("result"), py::arg("cubes_map"),
          "Quadratic coefficient (beta2) of close~1+t+t^2 in RTH.");
}
