...
void cube2mat_ret_var_q05(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            res_ptr[i*d1+j]=static_cast<float>(rets[k]);
        }
    }
}

void bind_cube2mat_ret_var_q05(py::module& m){
    m.def("cube2mat_ret_var_q05", &cube2mat_ret_var_q05,
          py::arg("result"), py::arg("cubes_map"),
          "5% quantile VaR of intraday log returns (RTH).");
}
