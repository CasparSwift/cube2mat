...
void cube2mat_std_vwap_dev_rel(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 0; t < d2; ++t) {
                ...
                if (std::isnan(c) || std::isnan(v) || v == 0.0f) continue;
                double dev = (static_cast<double>(c) - static_cast<double>(v)) / static_cast<double>(v);
                sum += dev;
                sumsq += dev * dev;
                ++cnt;
            }
...
void bind_cube2mat_std_vwap_dev_rel(py::module& m) {
    m.def("cube2mat_std_vwap_dev_rel", &cube2mat_std_vwap_dev_rel,
          py::arg("result"), py::arg("cubes_map"),
          "Std of (close-vwap)/vwap across RTH bars.");
}
