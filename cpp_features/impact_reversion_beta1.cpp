...
void cube2mat_impact_reversion_beta1(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
        ...
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 0; t < d2 - 1; ++t) {
                ...
                if (std::isnan(v) || std::isnan(p0) || std::isnan(p1) || !(p0 > 0.f) || !(p1 > 0.f))
                    continue;
                float y = (p1 - p0) / p0; // next simple return
                double x = v;
...
void bind_cube2mat_impact_reversion_beta1(py::module& m) {
    m.def("cube2mat_impact_reversion_beta1", &cube2mat_impact_reversion_beta1,
          py::arg("result"), py::arg("cubes_map"),
          "OLS slope of next simple return on current volume within 09:30–15:59.");
}
