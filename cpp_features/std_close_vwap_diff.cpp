...
void cube2mat_std_close_vwap_diff(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 0; t < d2; ++t) {
                ...
                double diff = static_cast<double>(c) - static_cast<double>(v);
                sum += diff;
                sumsq += diff * diff;
                ++cnt;
            }
...
void bind_cube2mat_std_close_vwap_diff(py::module& m) {
    m.def("cube2mat_std_close_vwap_diff", &cube2mat_std_close_vwap_diff,
          py::arg("result"), py::arg("cubes_map"),
          "Sample std of (close - vwap) within 09:30–15:59.");
}
