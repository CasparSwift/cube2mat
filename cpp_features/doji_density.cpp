...
void cube2mat_doji_density(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 1; t < d2; ++t) {
                ...
                ++count_valid;
                if (body / range <= theta) ++count_doji;
            }
            res_ptr[i * d1 + j] = (count_valid > 0)
                ? static_cast<float>(static_cast<double>(count_doji) / count_valid)
...
void bind_cube2mat_doji_density(py::module& m) {
    m.def("cube2mat_doji_density", &cube2mat_doji_density,
          py::arg("result"), py::arg("cubes_map"),
          "Fraction of bars classified as Doji (|body|/range<=0.1).");
}
