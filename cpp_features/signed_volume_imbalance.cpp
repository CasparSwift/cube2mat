...
void cube2mat_signed_volume_imbalance(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    auto cbuf=price_arr.request(); auto vbuf=vol_arr.request();
    if(cbuf.ndim!=3||vbuf.ndim!=3) throw std::runtime_error("arrays must be 3D");
...
#pragma omp parallel for collapse(2)
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            float num=0.0f, den=0.0f; float prev=std::numeric_limits<float>::quiet_NaN(); bool have=false;
            for(ssize_t t=0;t<d2;++t){
                float c=price_ptr[base+t];
                float v=vol_ptr[base+t];
                if(!std::isfinite(c)||!std::isfinite(v)) continue;
                if(have){
                    float ret=(c/prev)-1.0f;
                    float s= (ret>0)?1.0f:((ret<0)?-1.0f:0.0f);
                    num += s*v;
                }
                den += v;
                have=true; prev=c;
            }
            res_ptr[i*d1+j]=(den>0.0f && have)?(num/den):std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_signed_volume_imbalance(py::module& m){
    m.def("cube2mat_signed_volume_imbalance", &cube2mat_signed_volume_imbalance,
          py::arg("result"), py::arg("cubes_map"),
          "Signed interval_volume share based on return sign proxy");
}
