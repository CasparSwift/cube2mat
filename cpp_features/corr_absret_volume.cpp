...
void cube2mat_corr_absret_volume(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    if (pbuf.shape[0] != vbuf.shape[0] || pbuf.shape[1] != vbuf.shape[1] || pbuf.shape[2] != vbuf.shape[2]) {
        throw std::runtime_error("arrays must have the same shape");
    }
...
void bind_cube2mat_corr_absret_volume(py::module& m) {
    m.def("cube2mat_corr_absret_volume", &cube2mat_corr_absret_volume,
          py::arg("result"), py::arg("cubes_map"),
          "Pearson correlation between |log returns| and volume within session.");
}
