...
void cube2mat_close_15m_absret_share(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            double num = 0.0;
            for (size_t t = n - kk; t < n; ++t) num += std::fabs(r[t]);
...
void bind_cube2mat_close_15m_absret_share(py::module& m) {
    m.def("cube2mat_close_15m_absret_share", &cube2mat_close_15m_absret_share,
          py::arg("result"), py::arg("cubes_map"),
          "Share of absolute returns in last 15 minutes; ret by close.pct_change.");
}
