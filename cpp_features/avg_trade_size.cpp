...
void cube2mat_avg_trade_size(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    if (vbuf.shape[0]!=nbuf.shape[0] || vbuf.shape[1]!=nbuf.shape[1] || vbuf.shape[2]!=nbuf.shape[2])
        throw std::runtime_error("interval_volume and n shapes mismatch");
...
#pragma omp parallel for collapse(2)
    for (ssize_t i=0;i<d0;++i){
        for (ssize_t j=0;j<d1;++j){
            ...
            float sumv=0.0f; float sumn=0.0f;
            for (ssize_t t=0;t<d2;++t){
                float v=vol_ptr[base+t];
                float n=n_ptr[base+t];
                if(!std::isfinite(v)||!std::isfinite(n)) continue;
                sumv+=v; sumn+=n;
            }
            res_ptr[i*d1+j]=(sumn>0.0f)?(sumv/sumn):std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_avg_trade_size(py::module& m){
    m.def("cube2mat_avg_trade_size", &cube2mat_avg_trade_size,
          py::arg("result"), py::arg("cubes_map"),
          "Average shares per trade in RTH");
}
