...
void cube2mat_close_on_logn_beta(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    if (pbuf.ndim != 3 || nbuf.ndim != 3 || pbuf.shape[0]!=nbuf.shape[0] || pbuf.shape[1]!=nbuf.shape[1] || pbuf.shape[2]!=nbuf.shape[2]) {
        throw std::runtime_error("arrays must be 3D and have same shape");
    }
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            for (ssize_t t = 0; t < d2; ++t) {
                ...
                if (std::isnan(c) || std::isnan(n) || !(n > 0.f)) continue;
                x.push_back(std::log(static_cast<double>(n)));
...
void bind_cube2mat_close_on_logn_beta(py::module& m) {
    m.def("cube2mat_close_on_logn_beta", &cube2mat_close_on_logn_beta,
          py::arg("result"), py::arg("cubes_map"),
          "OLS slope of close on log(n) within 09:30–15:59; NaN if insufficient.");
}
