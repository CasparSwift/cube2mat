...
void cube2mat_volume_share_on_down(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
...
#pragma omp parallel for collapse(2)
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            float downv=0.0f, totv=0.0f; float prev=std::numeric_limits<float>::quiet_NaN(); bool have=false;
            for(ssize_t t=0;t<d2;++t){
                ...
                totv+=v; have=true; prev=c;
            }
            res_ptr[i*d1+j]=(totv>0.0f && have)?(downv/totv):std::numeric_limits<float>::quiet_NaN();
        }
    }
}

void bind_cube2mat_volume_share_on_down(py::module& m){
    m.def("cube2mat_volume_share_on_down", &cube2mat_volume_share_on_down,
          py::arg("result"), py::arg("cubes_map"),
          "Share of total interval_volume on down bars");
}
