...
void cube2mat_breakout_nextret_low(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            double sum = 0.0;
            int count = 0;
            float prev_low = std::numeric_limits<float>::infinity();
...
void bind_cube2mat_breakout_nextret_low(py::module& m) {
    m.def("cube2mat_breakout_nextret_low", &cube2mat_breakout_nextret_low,
          py::arg("result"), py::arg("cubes_map"),
          "Mean next simple return when making new session low.");
}
