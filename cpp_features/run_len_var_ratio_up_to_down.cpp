...
void cube2mat_run_len_var_ratio_up_to_down(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            if (ups.size() < 2 || dns.size() < 2) {
                res_ptr[i * d1 + j] = std::numeric_limits<float>::quiet_NaN();
                continue;
            }
            double mu_u = 0.0, mu_d = 0.0;
            for (int v : ups) mu_u += v; mu_u /= ups.size();
            for (int v : dns) mu_d += v; mu_d /= dns.size();
            double var_u = 0.0, var_d = 0.0;
            for (int v : ups) var_u += (v - mu_u)*(v - mu_u);
            for (int v : dns) var_d += (v - mu_d)*(v - mu_d);
            var_u /= (ups.size() - 1);
            var_d /= (dns.size() - 1);
            res_ptr[i * d1 + j] = (var_d > 0.0)
                ? static_cast<float>(var_u / var_d)
...
void bind_cube2mat_run_len_var_ratio_up_to_down(py::module& m) {
    m.def("cube2mat_run_len_var_ratio_up_to_down", &cube2mat_run_len_var_ratio_up_to_down,
          py::arg("result"), py::arg("cubes_map"),
          "Variance of up-run lengths divided by variance of down-run lengths (zeros break runs).");
}
