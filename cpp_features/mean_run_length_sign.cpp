...
void cube2mat_mean_run_length_sign(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            runs.push_back(cur);
            double sum = 0.0;
            for (int v : runs) sum += v;
            res_ptr[i * d1 + j] = runs.empty() ? std::numeric_limits<float>::quiet_NaN()
                                               : static_cast<float>(sum / runs.size());
        }
    }
}

void bind_cube2mat_mean_run_length_sign(py::module& m) {
    m.def("cube2mat_mean_run_length_sign", &cube2mat_mean_run_length_sign,
          py::arg("result"), py::arg("cubes_map"),
          "Average length of consecutive non-zero sign runs (RTH).");
}
