...
void cube2mat_relprice_log_elasticity_volume(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            for(ssize_t t=rth_start; t<=rth_end; ++t){
                ...
                if(std::isnan(c) || std::isnan(v) || !(c>0.f) || !(v>0.f)) continue;
                double x=std::log(v);
...
void bind_cube2mat_relprice_log_elasticity_volume(py::module& m){
    m.def("cube2mat_relprice_log_elasticity_volume", &cube2mat_relprice_log_elasticity_volume,
          py::arg("result"), py::arg("cubes_map"),
          "Elasticity: slope of log(close/anchor) on log(volume) within 09:30–15:59.");
}
