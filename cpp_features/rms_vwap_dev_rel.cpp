...
void cube2mat_rms_vwap_dev_rel(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for(ssize_t i=0;i<d0;++i){
        for(ssize_t j=0;j<d1;++j){
            ...
            for(ssize_t t=rth_start; t<=rth_end; ++t){
                ...
                if(std::isnan(c) || std::isnan(vw) || !(vw>0.f)) continue;
                double dev=(c - vw)/vw; sumsq += dev*dev; ++cnt;
            }
            res_ptr[i*d1+j]=(cnt>0)? static_cast<float>(std::sqrt(sumsq/cnt))
...
void bind_cube2mat_rms_vwap_dev_rel(py::module& m){
    m.def("cube2mat_rms_vwap_dev_rel", &cube2mat_rms_vwap_dev_rel,
          py::arg("result"), py::arg("cubes_map"),
          "sqrt(mean(((close-vwap)/vwap)^2)) across RTH bars.");
}
