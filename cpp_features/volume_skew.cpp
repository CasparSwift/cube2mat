...
void cube2mat_volume_skew(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            double mean = 0.0; for (float v : vols) mean += v; mean /= n;
            ...
            double adj = std::sqrt(n * (n - 1)) / (n - 2);
            res_ptr[i * d1 + j] = static_cast<float>(adj * g1);
        }
    }
}
...
void bind_cube2mat_volume_skew(py::module& m) {
    m.def("cube2mat_volume_skew", &cube2mat_volume_skew,
          ...
          "Skewness of volume distribution across RTH bars.");
}
