...
static bool haar_levels(std::vector<double> a, int L, std::vector<double>& out) {
    for (int lvl = 0; lvl < L; ++lvl) {
        ...
        for (size_t i = 0; i < n/2; ++i) {
            next[i] = (a[0 + 2*i] + a[1 + 2*i]) / std::sqrt(2.0);
        }
        a.swap(next);
    }
    out = a;
...
void cube2mat_wavelet_energy_lowfreq_share(py::array_t<float, py::array::c_style | py::array::forcecast>& result,
    ...
    for (ssize_t i = 0; i < d0; ++i) {
        for (ssize_t j = 0; j < d1; ++j) {
            ...
            int maxL = std::min<int>(L, std::floor(std::log2(std::max<size_t>(2,n))));
            std::vector<double> aL; if(!haar_levels(e,maxL,aL) || aL.empty()){res_ptr[i*d1+j]=std::numeric_limits<float>::quiet_NaN(); continue;}
            double low=0.0; for(double v:aL) low+=v*v;
            double val = low / tot; if(val<0) val=0; if(val>1) val=1;
...
void bind_cube2mat_wavelet_energy_lowfreq_share(py::module& m) {
    m.def("cube2mat_wavelet_energy_lowfreq_share", &cube2mat_wavelet_energy_lowfreq_share,
          py::arg("result"), py::arg("cubes_map"),
          "Haar-DWT low-frequency energy share of detrended close (L=3).");
}
